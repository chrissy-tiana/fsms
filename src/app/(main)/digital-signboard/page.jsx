...
function DigitalSignboardManagement() {
  ...
  const getPriceChange = (currentPrice, history) => {
    if (!history || history.length < 2) return { change: "0.00", percentage: "0.00" };
          ...
          <CardContent>
            <div className="text-sm font-bold">
              {currentPrices.length > 0 
                ? new Date(
                    Math.max(...currentPrices.map(p => new Date(p.effectiveDate).getTime()))
                  ).toLocaleString()
                : "No updates"
              }
            </div>
              ...
              {currentPrices.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {currentPrices.map((price) => {
                    ...
                    const priceChange = getPriceChange(price.price, fuelHistory);
                    
                    return (
                      <div key={price.fuelType} className="p-4 border rounded-lg">
                        ...
                        <div className="text-center">
                          ...
                          <div className="text-xs text-muted-foreground mt-2">
                            Updated: {new Date(price.effectiveDate).toLocaleString()}
                          </div>
                          {price.updatedBy && (
                            <div className="text-xs text-muted-foreground">
                              By: {price.updatedBy}
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm">Add fuel prices to display on signboard</p>
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <CardContent>
              <form onSubmit={handleUpdatePrice} className="space-y-4">
                ...
                <div className="flex gap-2 pt-4">
                  <Button type="submit" className="bg-black text-white" disabled={updating}>
                    {updating ? (
              ...
              {priceForm.fuelType && priceForm.newPrice && (
                <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                  ...
                  {(() => {
                    ...
                    const percentage = currentPrice > 0 ? ((change / currentPrice) * 100).toFixed(2) : "0.00";
                ...
                <Table>
                  ...
                  <TableBody>
                    {priceHistory.map((entry, index) => (
                      <TableRow key={`${entry.fuelType}-${entry.effectiveDate}-${index}`}>
                        <TableCell>
